...
class Trainer:
    ...
    def train(self) -> None:
        ...
        for e in range(self.args.epochs):
            ...
            with tqdm(total=len(train_loader), desc=f"Train epoch {e + 1}/{self.args.epochs}") as pbar:
            ...
            running_loss = 0
            predictions, targets = torch.LongTensor([]), torch.LongTensor([])
            with tqdm(total=len(test_loader), desc=f"Test epoch {e + 1}/{self.args.epochs}") as pbar:
