...
def add_noise_to_dataset(dataset: Dataset, std: float = 0.3) -> TensorDataset:
    ...
    Y = torch.cat([torch.ones(size=(Y.size(0),), dtype=torch.long), *[torch.zeros(size=(Y.size(0) // 2,), dtype=torch.long)] * 2], dim=0)
...
training_args = TrainingArguments(
    ...
    seed=1337,
    batch_size=512,
    validation_intervall=ceil(len(train_dataset) / 512),
    no_cuda=True
)
