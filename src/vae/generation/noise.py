...
class Noise:
    def __init__(self, alpha: float, k: int, std: float, return_indices: bool = False, indices_before: Optional[Tensor] = None) -> None:
        self.alpha = alpha
        self.k = k
        self.std = std
    ...
    def __call__(self, z: Tensor, y: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        indices_before = torch.tensor(self.indices_before) if self.indices_before is not None else torch.arange(0, z.size(0), 1)
