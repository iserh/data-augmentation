...
class VAETrainer(Trainer):
    def train_epoch(self, epoch: int, train_loader: DataLoader, pbar: tqdm) -> Dict[str, float]:
        ...
        for step, (x, _) in enumerate(train_loader, start=1):
            ...
            bce_l, kl_l = self.train_step(x.to(self.device, non_blocking=True))["loss"]
            running_bce_l += bce_l
            running_kl_l += kl_l
            # log to mlflow
    ...
    def test_epoch(self, epoch: int, test_loader: DataLoader) -> Dict[str, float]:
        ...
        with tqdm(total=len(test_loader), desc=f"Test epoch {epoch}/{self.args.epochs}", leave=False) as test_pbar:
            for step, (x, _) in enumerate(test_loader, start=1):
                # test step, update running loss
                bce_l, kl_l = self.test_step(x.to(self.device, non_blocking=True))["loss"]
                running_bce_l += bce_l
                running_kl_l += kl_l
                # log to mlflow
